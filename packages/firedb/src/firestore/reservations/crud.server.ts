...
type ReserveConverter = FirestoreDataConverter<
  m.ReservationAppModel,
  m.ReservationDbModel
>;
...
const firestoreConverter: ReserveConverter = {
  ...
  fromFirestore: (snapshot: QueryDocumentSnapshot<m.ReservationDbModel>) => {
    const createdDateDoc = snapshot.data().createdDate;
    const createdDate = createdDateDoc
      ? createdDateDoc.toDate()
